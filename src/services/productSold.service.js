...
const getAllSoldProducts = async () => {
    ...
    for (const item of orderItems) {
      const productId = item.productId?._id ? item.productId._id.toString() : 'unknown';
  ...
  const getSoldProductAnalytics = async () => {
    ...
    const inStockCount = series.filter(s => s.isAvailable && s.quantity > 0).length;
    const soldOutCount = series.filter(s => !s.isAvailable || s.quantity <= 0).length;
